...
//! RISC-V Sv48 page-table entries.
//!
//! # Verified Properties
//!
//! The paging proofs establish PTE encoding, address extraction, property round
//! trips, and level-dependent well-formedness. Verified setters require readable
//! mappings, Writeback or Uncacheable caching, and clear input A/D flags because
//! the current implementation clears those bits. CSR operations and TLB effects
//! remain outside these proofs; see `docs/riscv-verification.md`.
use alloc::fmt;
use core::ops::Range;
use vstd::{
...
use crate::{
    Pod,
    mm::{
        CurrentPagingConstsTrait, MAX_PADDR, Paddr, PagingConstsTrait, PagingLevel, PodOnce, Vaddr,
...
pub unsafe fn activate_page_table(root_paddr: Paddr, _root_pt_cache: CachePolicy) {
    ...
    // SAFETY: The caller guarantees that the new mappings preserve memory safety.
    unsafe { riscv::register::satp::set(riscv::register::satp::Mode::Sv48, 0, ppn) };
}
...
impl PageTableEntryTrait for PageTableEntry {
    ...
    open spec fn new_page_req(_paddr: Paddr, _level: PagingLevel, prop: PageProperty) -> bool {
        &&& prop.inv()
        // The current implementation clears A/D when setting properties.
        &&& prop.flags.bits() & 0x18u8 == 0
            ...
            forall|level: PagingLevel| final(self).is_last(level),
    {
        proof {
            lemma_riscv_flag_constants();
            lemma_riscv_encode_without_ad(prop.flags.bits(), prop.priv_flags.bits());
        }
    ...
    fn is_last(&self, level: PagingLevel) -> bool {
        let rwx = PageTableFlags::READABLE().bits() | PageTableFlags::WRITABLE().bits()
    ...
    proof fn lemma_page_table_entry_properties() {
        ...
        assert forall|level: PagingLevel| 1 < level implies !(#[trigger] Self::new_absent().is_last(
            level,
        )) by {
...
closed spec fn encode_riscv_prop_bits_spec(
    ...
    0x1usize | if pbits & 0x1usize != 0 {
        ...
        0
    } | if pbits & 0x10usize != 0 {
        0x80usize
    } else {
        0
    } | if pbits & 0x40usize != 0 {
        0x100usize
    } else {
        0
    } | if pbits & 0x80usize != 0 {
        0x200usize
    } else {
        0
    } | if priv_bits & 0x1usize != 0 {
        0x10usize
    } else {
        0
    } | if priv_bits & 0x2usize != 0 {
        0x20usize
    } else {
        0
    } | if uncacheable {
...
proof fn lemma_riscv_shifted_flags_match(raw: usize)
    ensures
        (((raw & 0x2usize) >> 1 << 0) | ((raw & 0x4usize) >> 2 << 1) | ((raw & 0x8usize) >> 3 << 2)
            | ((raw & 0x40usize) >> 6 << 3) | ((raw & 0x80usize) >> 7 << 4) | ((raw & 0x100usize)
            >> 8 << 6) | ((raw & 0x200usize) >> 9 << 7)) == decode_riscv_page_bits_spec(raw),
...
proof fn lemma_riscv_encode_bits_expanded(pbits: u8, priv_bits: u8, uncacheable: bool)
    ensures
        (0x1usize | ((pbits as usize & 0x1usize) >> 0 << 1) | ((pbits as usize & 0x2usize) >> 1
            << 2) | ((pbits as usize & 0x4usize) >> 2 << 3) | ((pbits as usize & 0x8usize) >> 3
            << 6) | ((pbits as usize & 0x10usize) >> 4 << 7) | ((pbits as usize & 0x40usize) >> 6
            << 8) | ((pbits as usize & 0x80usize) >> 7 << 9) | ((priv_bits as usize & 0x1usize) >> 0
            << 4) | ((priv_bits as usize & 0x2usize) >> 1 << 5) | if uncacheable {
            0x4000_0000_0000_0000usize
        } else {
            0usize
...
// The existing setter clears A/D, so its round trip requires both input bits clear.
#[verifier::bit_vector]
proof fn lemma_riscv_encode_without_ad(pbits: u8, priv_bits: u8)
    requires
        pbits & 0x18u8 == 0,
    ensures
        (0x1usize | ((pbits as usize & 0x1usize) >> 0 << 1) | ((pbits as usize & 0x2usize) >> 1
            << 2) | ((pbits as usize & 0x4usize) >> 2 << 3) | ((pbits as usize & 0x40usize) >> 6
            << 8) | ((pbits as usize & 0x80usize) >> 7 << 9) | ((priv_bits as usize & 0x1usize) >> 0
            << 4) | ((priv_bits as usize & 0x2usize) >> 1 << 5)) == encode_riscv_prop_bits_spec(
            pbits as usize,
            priv_bits as usize,
            false,
        ),
        encode_riscv_prop_bits_spec(pbits as usize, priv_bits as usize, false)
            | 0x4000_0000_0000_0000usize == encode_riscv_prop_bits_spec(
            pbits as usize,
            priv_bits as usize,
            true,
        ),
...
{
    lemma_riscv_flag_constants();
    assert(parse_flags!(raw, PageTableFlags::READABLE(), PageFlags::R()) == ((raw & 0x2usize) >> 1
        << 0)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::WRITABLE(), PageFlags::W()) == ((raw & 0x4usize) >> 2
        << 1)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::EXECUTABLE(), PageFlags::X()) == ((raw & 0x8usize) >> 3
        << 2)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::ACCESSED(), PageFlags::ACCESSED()) == ((raw
        & 0x40usize) >> 6 << 3)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::DIRTY(), PageFlags::DIRTY()) == ((raw & 0x80usize) >> 7
        << 4)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::RSV1(), PageFlags::AVAIL1()) == ((raw & 0x100usize)
        >> 8 << 6)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::RSV2(), PageFlags::AVAIL2()) == ((raw & 0x200usize)
        >> 9 << 7)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::USER(), PrivFlags::USER()) == ((raw & 0x10usize) >> 4
        << 0)) by (compute);
    assert(parse_flags!(raw, PageTableFlags::GLOBAL(), PrivFlags::GLOBAL()) == ((raw & 0x20usize)
        >> 5 << 1)) by (compute);
...
proof fn lemma_riscv_encode_prop_matches(pbits: u8, priv_bits: u8, uncacheable: bool)
    ensures
        (PageTableFlags::VALID().bits()
...
proof fn lemma_riscv_new_paddr_bits(paddr: Paddr)
    ensures
        ...
        paddr < 0x8000_0000usize ==> {
            let raw = (paddr >> 12) << 10 | 0x1usize;
            ((raw & PageTableEntry::PHYS_ADDR_MASK) >> 10) << 12 == paddr & !0xFFFusize
        },
        ({
            let raw = (paddr >> 12) << 10 | 0x1usize;
...
proof fn lemma_riscv_set_prop_roundtrip(old_raw: usize, prop: PageProperty)
    ...
    ensures
        ...
        PageTableEntry(PageTableEntry::raw_set_prop_spec(old_raw, prop)).paddr() == PageTableEntry(
            old_raw,
        ).paddr(),
        ...
        forall|level: PagingLevel| #[trigger]
            PageTableEntry(old_raw).is_last(level) ==> PageTableEntry(
...
proof fn lemma_riscv_new_page_properties(paddr: Paddr, level: PagingLevel, prop: PageProperty)
    requires
        PageTableEntry::new_page_req(paddr, level, prop),
    ensures
        PageTableEntry::new_page(paddr, level, prop).is_present(),
        paddr < MAX_PADDR ==> PageTableEntry::new_page(paddr, level, prop).paddr() == paddr & !((
        PAGE_SIZE - 1) as usize),
        paddr < MAX_PADDR && paddr % PAGE_SIZE == 0 ==> PageTableEntry::new_page(
            paddr,
            level,
            prop,
        ).paddr() == paddr,
        ...
        forall|level: PagingLevel| #[trigger]
            PageTableEntry::new_page(paddr, level, prop).is_last(level),
{
    lemma_riscv_flag_constants();
    lemma_riscv_new_paddr_bits(paddr);
...
proof fn lemma_riscv_new_pt_properties(paddr: Paddr)
    ensures
        ...
        paddr < MAX_PADDR ==> PageTableEntry::new_pt(paddr).paddr() == paddr & !((PAGE_SIZE
            - 1) as usize),
...
pub proof fn lemma_riscv_new_page_pte_wf(paddr: Paddr, level: PagingLevel, prop: PageProperty)
    requires
        paddr < MAX_PADDR,
        paddr % PAGE_SIZE == 0,
        1 <= level <= NR_LEVELS,
...
pub proof fn lemma_riscv_set_prop_preserves_pte_wf(
    ...
    requires
        ...
        prop.cache is Writeback || prop.cache is Uncacheable,
    ensures
        PageTableEntry::from_raw_spec(PageTableEntry::raw_set_prop_spec(old_raw, prop)).pte_wf(
            level,
        ),
        PageTableEntry::from_raw_spec(
            PageTableEntry::raw_set_prop_spec(old_raw, prop),
        ).has_tracked_tag() == prop.flags.contains(PageFlags::AVAIL1()),
        (prop.flags.contains(PageFlags::AVAIL1()) == PageTableEntry::from_raw_spec(
            old_raw,
        ).has_tracked_tag()) ==> PageTableEntry::from_raw_spec(
            PageTableEntry::raw_set_prop_spec(old_raw, prop),
