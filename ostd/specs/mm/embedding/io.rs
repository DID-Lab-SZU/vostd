...
pub proof fn vm_writer_limit_embedded(tracked owner: &mut VmIoOwner, _max_avail: usize)
    requires
        old(owner).inv(),
    ensures
        final(owner).inv(),
...
pub proof fn vm_writer_skip_embedded(tracked owner: &mut VmIoOwner, _nbytes: usize)
    requires
        old(owner).inv(),
    ensures
        final(owner).inv(),
