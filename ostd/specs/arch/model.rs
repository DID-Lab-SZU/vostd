...
use vstd::{
    arithmetic::power2::{lemma2_to64_rest, pow2},
    prelude::*,
};
...
pub open spec fn is_canonical_vaddr_for<C: PagingConstsTrait>(va: Vaddr) -> bool {
    if C::VA_SIGN_EXT() {
        va < pow2((C::ADDRESS_WIDTH() - 1) as nat) || usize::MAX - pow2(
            (C::ADDRESS_WIDTH() - 1) as nat,
        ) < va
    } else {
...
{
    lemma2_to64_rest();
    assert(pow2(47) == 0x8000_0000_0000int);
    assert(usize::MAX == 0xffff_ffff_ffff_ffffint) by (compute_only);
}
